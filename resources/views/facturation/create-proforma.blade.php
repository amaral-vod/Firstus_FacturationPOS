...
<form action="{{ route('facturation.store') }}" method="POST" id="proforma-form">
@csrf
<div class="row g-3">
    <div class="col-lg-8">
        <div class="card card-modern mb-3">
                    ...
                    <tbody id="items-body">
                        @if($vente)
                            ...
                            <tr class="item-row">
                                ...
                                <td class="line-total align-middle">{{ number_format($d->total_ligne, 0, ',', ' ') }}</td>
                            ...
                            <tr class="item-row">
                                <td>
                                    <select class="form-select form-select-sm product-select mb-1">
                                        ...
                                        <option value="{{ $p->id }}" data-name="{{ $p->name }}" data-price="{{ $p->effective_price }}">{{ $p->name }} ({{ number_format($p->effective_price,0,',',' ') }})</option>
                                        @endforeach
                                    </select>
            ...
            <div class="card-footer bg-white">
                <button type="button" class="btn btn-sm btn-outline-primary" id="add-row">+ Ajouter une ligne</button>
            </div>
        </div>
    </div>
    ...
    <div class="col-lg-4">
        <div class="card card-modern mb-3">
            ...
            <div class="card-body">
                ...
                <input type="text" name="client_adresse" id="client_adresse" class="form-control mb-2" placeholder="Adresse">
                <input type="text" name="client_telephone" id="client_telephone" class="form-control" placeholder="Téléphone">
            </div>
        </div>

        <div class="card card-modern mb-3">
            <div class="card-header bg-white fw-semibold">Options</div>
            <div class="card-body">
                <label class="form-label">Format papier</label>
                <select name="format_papier" class="form-select mb-2">
                    <option value="auto">🔄 Automatique (selon contenu)</option>
                    <option value="ticket">🧾 Ticket 80 mm</option>
                    <option value="a5">📄 A5</option>
                    <option value="a4">📃 A4</option>
                </select>
                <p class="small text-muted mb-2">Auto : peu de lignes → ticket ou A5 ; beaucoup de lignes → A4.</p>
                ...
                <input type="number" name="remise" id="remise" class="form-control mb-2" value="0" min="0" step="0.01">
                <label class="form-label">Notes</label>
                <textarea name="notes" class="form-control" rows="3" placeholder="Conditions, délais..."></textarea>
            </div>
        </div>
        ...
        <div class="card card-modern">
            <div class="card-body">
                ...
                <div class="d-flex justify-content-between fs-5"><span>Total estimé</span><strong id="grand-total">0 FCFA</strong></div>
